...
Your judgment:"""
        ...
        if response:
            ...
            if 'SURVIVED' in response_upper:
                ...
                reason = parts[1].strip() if len(parts) > 1 else "Their plan was adequate"
                return {'survived': True, 'reason': reason}
            elif 'DIED' in response_upper:
                # Extract reason after DIED:
                parts = response.split(':', 1)
                reason = parts[1].strip() if len(parts) > 1 else "Their plan was inadequate"
        ...
        return {
            'survived': survived,
            'reason': "Detailed planning saved them" if survived else "Plan lacked detail"
        }
